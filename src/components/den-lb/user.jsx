import { calculateUserTotalLb, db, formatPrice, deleteDocumentWithBackup } from "../../firebase";
    ...
    const addDebit = (e) => {
        ...
        if (parseInt(debit) < 1) {
            ...
            if (window.confirm('Sure?')) {
                dispatch(show());
                const userRef = doc(db, 'users-lb', user.id);
                addDoc(collection(db, "debits-lb"), {
                    user: userRef,
                    ...
                    .finally(() => {
                        calculateUserTotalLb(userRef, props.update);
                    });
            }
        }
    }
        ...
        if (window.confirm('Sure?')) {
            ...
            for (let index = 0; index < debits.length; index++) {
                ...
                if (refEqual(debit.user, user.userRef)) {
                    const docRef = doc(db, 'debits-lb', debit.id);
            ...
            for (let index = 0; index < paids.length; index++) {
                ...
                if (refEqual(paid.user, user.userRef)) {
                    const docRef = doc(db, 'paids-lb', paid.id);
                    await new Promise(resolve => deleteDoc(docRef)
                        .then(() => {
            ...
            getDocs(query(collection(db, 'debits-lb'), where('user', '==', user.userRef)))
                .then((d) => {
                    if (d.docs.length === 0) {
                        getDocs(query(collection(db, 'paids-lb'), where('user', '==', user.userRef)))
                            .then((d) => {
                                if (d.docs.length === 0) {
                                    deleteDocumentWithBackup('users-lb', user, props.update);
                                } else {
                                ...
                                .then((d) => {
                                    if (d.docs.length === 0) {
                                        getDocs(query(collection(db, 'paids-lb'), where('user', '==', user.userRef)))
                                            .then((d) => {
                                                if (d.docs.length === 0) {
                                                    calculateUserTotalLb(user.userRef, props.update);
                                                } else {
